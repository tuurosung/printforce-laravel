<div class="modal fade" id="editEmbroideryJobModal" tabindex="-1" role="dialog" aria-labelledby="modalTitleId"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitleId">
                    Edit Embroidery Job
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form method="POST" action="{{ route('jobs.embroidery.update', $embroideryJob) }}" autocomplete="off">
                @csrf

                <div class="modal-body">

                    <section class="mb-4">

                        <h6 class="mb-3">Customer Details</h6>

                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="customer_name" class="form-label">Customer Name</label>
                                    <input type="text" class="form-control" name="customer_name" id="customer_name"
                                        readonly value="{{ $embroideryJob->customer->name }}" />
                                </div>

                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="date" class="form-label">Date</label>
                                    <input type="text" class="form-control" name="date" id="date"
                                        aria-describedby="helpId" placeholder="" value="{{ $embroideryJob->date }}" />
                                </div>

                            </div>
                        </div>


                    </section>

                    <section class="mb-4">

                        <h6 class="mb-3">Job Details</h6>

                        <div class="row">
                            <div class="col-md-6">

                                <div class="mb-3">
                                    <label for="service_id" class="form-label">Service Name</label>
                                    <select class="form-select" name="service_id" id="service_id"
                                        data-customer_id="{{ $embroideryJob->customer_id }}"
                                        data-fetch-url="{{ route('configuration.print-services.get-service-cost-with-customer-id') }}"
                                        required>
                                        <option value="">Select Option</option>
                                        @foreach ($embroidery_services as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ $embroideryJob->service_id === $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>

                            </div>
                            <div class="col-md-6">
                                <x-printforce.inputs.text-input name="unit_cost" id="embroidery_unit_cost"
                                    label="Unit Cost" placeholder="0.00" :value="$embroideryJob->unit_cost"
                                    readonly="true" />
                            </div>
                        </div>
                    </section>

                    <section class="mb-4">

                        <h6 class="mb-3">Production Details</h6>

                        <div class="p-3 bg-light rounded-3">

                            <div class="row">
                                <div class="col-md-6">
                                    <x-printforce.inputs.text-input name="qty" id="embroidery_qty" label="Quantity"
                                        placeholder="1" :value="$embroideryJob->qty" required />
                                </div>
                                <div class="col-md-6">
                                    <x-printforce.inputs.text-input name="embroidery_cost" id="embroidery_cost"
                                        label="Embroidery Cost" placeholder="0.00"
                                        :value="$embroideryJob->embroidery_cost" readonly="true" />
                                </div>
                            </div>

                            <div class="row">
                                <div class="col-6">
                                    <div class="mb-3">
                                        <label for="" class="form-label">Material Purchase</label>
                                        <select class="form-select form-select-sm" name="mat_supply" id="mat_supply">
                                            <option value="">-- Who is buying the materials? --</option>
                                            @foreach(config('printforce.printServices.material_supply_options') as $key => $value)
                                            <option value="{{ $key }}"
                                                {{ $embroideryJob->mat_supply == $key ? 'selected' : '' }}>
                                                {{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <x-printforce.inputs.text-input name="mat_unit_cost" id="mat_unit_cost"
                                        label="Material Unit Cost" placeholder="0.00"
                                        value="{{ $embroideryJob->mat_unit_cost }}" required />
                                </div>
                            </div>

                            <div class="row">
                                <div class="col-6">
                                    <x-printforce.inputs.text-input name="purchase_cost"
                                        id="embroidery_purchase_cost" label="Material Purchase Cost"
                                        placeholder="0.00" value="{{ $embroideryJob->purchase_cost }}" readonly
                                        required />
                                </div>
                                <div class="col-md-6">
                                    <x-printforce.inputs.text-input name="total" id="embroidery_total"
                                        label="Overall Cost" value="{{ $embroideryJob->total }}"
                                        placeholder="0.00" readonly />
                                </div>
                            </div>

                        </div>

                    </section>

                    <section class="mb-4">
                        <h6 class="mb-3">Additional Details</h6>
                        <x-printforce.inputs.text-input name="notes" id="em_notes"
                                        label="Notes (optional eg; CRS Farmers T-shirts)" placeholder=""
                                        :value="$embroideryJob->notes" />
                    </section>

                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                        Close
                    </button>
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-check me-3  "></i>
                        Update Embroidery Job
                    </button>
                </div>
            </form>
        </div>
    </div>
</div>
