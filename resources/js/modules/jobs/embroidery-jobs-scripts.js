...
$(document).on('click', '.table tbody .edit', function (event) {
    ...
    $.get(url, function (response) {
        ...
        $('#edit_embroidery_job_modal').modal('show');
        ...
        $('#embroidery_qty').on('keyup', function (event) {
            calculateEmbroideryTotal();
        })
        ...
        $('#mat_supply').on('change', function (event) {
            ...
            if ($(this).val() === 'yes') {
                ...
                $('#mat_unit_cost').val('0').prop('readonly', true);
            }
...
/**
 * Calculate the total cost of the embroidery job
 * based on the quantity, unit cost, and material supply.
 * It updates the total cost, purchase cost, and embroidery cost fields.
 */
