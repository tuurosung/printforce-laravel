...
class PressJobService implements PressJobServiceInterface
{
    ...
    public function __construct(
        private PressJob $model
    ){}


    public function create(Customer $customer, array $data): PressJob
    {
        $pressJob = $customer->pressJobs()->create($data);

        if (! $pressJob) {
            $error = "Unable to create new press job";
            Log::error($error);
    ...
    public function update(PressJob $pressJob, array $data): bool
    {
        $isUpdated = $pressJob->update($data);

        if (! $isUpdated) {
            $error = "Unable to update Press Job";
            Log::error($error);
            throw new \Exception($error);
        }
    ...
    {
        $isDeleted = $pressJob->delete();

        if (! $isDeleted) {
            $error = "Unable to delete Press Job";
            Log::error($error);
            throw new \Exception($error);
        }
