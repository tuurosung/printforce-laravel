...
class DesignJobService implements DesignJobServiceInterface
{
    ...
    public function __construct(
        private DesignJob $model
    ){}


    public function create(Customer $customer, array $data): DesignJob
    {
        $designJob = $customer->designJobs()->create($data);

        if (! $designJob) {
            $error = "Unable to create design job";
    ...
    {
        $isUpdated = $designJob->update($data);

        if (! $isUpdated) {
            $error = "Unable to update design job";
            Log::error($error, [$data]);
            throw new \Exception($error);
        }
    ...
    {
        $isDeleted = $designJob->delete();

        if (! $isDeleted) {
            $error = "Unable to delete design job";
            Log::error($error, [$designJob]);
            throw new \Exception($error);
        }
